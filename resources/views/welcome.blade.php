    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <section class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            @forelse ($daftarRumahSakit as $rs)
                <a href="http://{{ $rs->domain }}" class="block bg-white rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300">
                    <div class="p-6">
                        <div class="flex items-center">
                            @if ($rs->logo)
                                <img src="{{ Storage::url($rs->logo) }}" alt="Logo {{ $rs->name }}" class="h-16 w-16 object-contain mr-4">
                            @else
                                <div class="h-16 w-16 bg-gray-200 rounded-md flex items-center justify-center mr-4">
                                    <span class="text-gray-500">No Logo</span>
                                </div>
                            @endif
                            <h2 class="text-xl font-bold text-gray-900">{{ $rs->name }}</h2>
                        </div>
                    </div>
                </a>
            @empty
